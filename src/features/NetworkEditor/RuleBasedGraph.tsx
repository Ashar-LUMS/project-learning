...
export function RuleBasedGraph({ 
  ...
  // Cytoscape refs for weight-based visualization
 ...
 const handleBooleanRulesSubmit = (submittedRules: string[]) => {
  ...
  // Initialize and manage Cytoscape for weight-based graph visualization
  useEffect(() => {
    if (!booleanNetwork || !cyContainerRef.current) return;

    if (!cyRef.current) {
      // Convert Boolean network to Cytoscape format
      const cyElements = [
        ...booleanNetwork.nodes.map(node => ({
          data: {
            id: node.id,
            label: node.label || node.id,
            state: nodeStates[node.id] ? 'active' : 'inactive',
          },
        })),
        ...booleanNetwork.edges.map((edge, idx) => ({
          data: {
        ...
        style: [
          {
            selector: 'node',
            style: {
              'background-color': (ele: any) => ele.data('state') === 'active' ? '#10b981' : '#d1d5db',
              'label': 'data(label)',
              'color': (ele: any) => ele.data('state') === 'active' ? '#fff' : '#374151',
              'font-size': 12,
              'text-valign': 'center',
              'text-halign': 'center',
              'width': 40,
              'height': 40,
              'border-width': 2,
              'border-color': (ele: any) => ele.data('state') === 'active' ? '#059669' : '#9ca3af',
            },
          },
          {
            selector: 'edge',
            style: {
              'width': 2,
              'line-color': (ele: any) => ele.data('edgeType') === 'activation' ? '#10b981' : '#ef4444',
              'target-arrow-color': (ele: any) => ele.data('edgeType') === 'activation' ? '#10b981' : '#ef4444',
      ...
      const layout = cyRef.current.layout({ name: 'breadthfirst', directed: true, animate: false });
      layout.run();
  ...
  useEffect(() => {
    if (cyRef.current) {
      booleanNetwork?.nodes.forEach(node => {
        ...
        if (cyNode) {
          cyNode.data('state', nodeStates[node.id] ? 'active' : 'inactive');
        }
      });
    }
  }, [nodeStates, booleanNetwork]);

  // Cleanup Cytoscape and interval on unmount
  useEffect(() => {
    return () => {
      if (simulationInterval) {
        clearInterval(simulationInterval);
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
                ...
                <TabsContent value="graph" className="h-[calc(100%-60px)] mt-0">
                  <div className="h-full border rounded-lg bg-white overflow-hidden flex flex-col">
                    <div ref={cyContainerRef} className="flex-1" />
                    <div className="p-3 border-t bg-gray-50 flex items-center justify-between">
                      <div className="flex gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs">Inhibition</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => {
                          if (cyRef.current) {
                            const layout = cyRef.current.layout({ name: 'breadthfirst', directed: true });
                            layout.run();
                          }
                        }}>Reflow</Button>
                      </div>
