...
export async function inferRulesFromBiomolecules(biomolecules: string[]) {
  ...
  if (!trimmed.length) {
    ...
    const systemPrompt = `You are a biomedical network inference assistant. Use only the biomolecules provided by the user. Do not invent or infer new entities, phenotypes, diseases, or pathways. If a rule would require a biomolecule that was not supplied, you must omit that rule rather than introducing a new identifier. Produce boolean-style regulatory rules with the following requirements:\n- Exactly one rule per target node\n- Each rule must use the syntax: TARGET = EXPRESSION\n- EXPRESSIONS may include the operators && (AND), || (OR), and ! (NOT) as well as parentheses\n- Only the provided biomolecule identifiers may appear in the rules\n- Output only the rules, no commentary or numbering.`;  

    const userPrompt = `Biomolecules (only use these names in your rules, and reject any other identifiers):\n${trimmed.map((name) => `- ${name}`).join("\n")}\n\nGenerate one rule per target node using the format TARGET = EXPRESSION, where the expression can combine the biomolecules with && (AND), || (OR), ! (NOT), and parentheses. Every TARGET and every identifier inside EXPRESSION must exactly match one of the biomolecules listed above. If an identifier is not available, exclude that rule. Do not include numbering, bullet points, or any text outside the rules.`;  
